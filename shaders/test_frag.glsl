...
#define NUM_LIGHTS 2  // Define the number of light sources
...
void main()
{
    ...
    vec3 normal = normalize(Normal);  // Default normal

    // If using normal maps, perturb the normal with tangent space
    vec3 N = normalize(normal);  // N is the perturbed normal if normal mapping is used

    vec3 viewDir = normalize(viewPos - FragPos);  // Vector from fragment to camera

    // Calculate F0 based on metallic and albedo
    vec3 F0 = vec3(0.04);  // Default reflectivity for non-metallic surfaces
